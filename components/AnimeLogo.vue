...
export default {
  mounted() {
    ...
    var bouncePath = this.$anime.path(".bounce path");
    this.fitElementToParent(logoAnimationEl, 0, ".bounce svg");
    this.$anime.set([".letter-a", ".letter-n", ".letter-i"], { translateX: 70 });
    this.$anime.set(".letter-e", { translateX: -70 });
    this.$anime.set(".dot", { translateX: 630, translateY: -200 });
    var logoAnimationTL = this.$anime
      .timeline({
        autoplay: true,
        easing: "easeOutSine",
        loop: true
      ...
      .add(
        {
          ...
          duration: 800,
          delay: this.$anime.stagger(40, { from: 2.5 }),
          change: function(a) {
      ...
      .add(
        {
          ...
          delay: this.$anime.stagger(60, { from: "center" })
        },
        "-=670"
      )
      .add(
  ...
  methods: {
    fitElementToParent(el, padding, exception) {
      const self = this;
      var timeout = null;
      function resize() {
        if (timeout) clearTimeout(timeout);
        self.$anime.set(el, { scale: 1 });
        if (exception) self.$anime.set(exception, { scale: 1 });
        var pad = padding || 0;
        ...
        timeout = setTimeout(function() {
          self.$anime.set(el, { scale: ratio });
          if (exception) self.$anime.set(exception, { scale: invertedRatio });
        }, 10);
      }
      resize();
